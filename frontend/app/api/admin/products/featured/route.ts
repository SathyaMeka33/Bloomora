...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!productId || typeof isFeatured !== 'boolean') {
      return NextResponse.json({ success: false, error: 'productId and boolean isFeatured are required' }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, message: `Product featured flag set to ${isFeatured}` });
  } catch (error: any) {
